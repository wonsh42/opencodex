...
visible content. The bundled MiniMax registry still needs its owner to populate
that list for all eight MiniMax models; explicit config already reaches the
adapter correctly.
...
| OpenAI Chat | Complete: history, tools, provider restrictions, split/adaptive reasoning, reasoning order, malformed preflight, EOF and usage | Wired by `openai.NewChatAdapter`; `ReasoningSplitModels` is consumed at request construction | Strong partial: Chat route/error and provider-default wire selection are strict; advanced transforms are less extensive than Responses | Shared OpenAI corpus: 2 targets, 13 seeds across Chat and Responses | Package race, malformed/large/read-failure matrix, 12,000-event soak | 116,974 B/op; 1,738 allocs/op | Adapter complete; bundled MiniMax capability seed remains registry-owner work |
...
## Remaining cross-scope integration

The only adapter-related residual is registry population, not adapter logic.
The shared config owner has added and round-trip-tested
`ProviderConfig.ReasoningSplitModels`. The provider/registry owner must now seed
the eight TypeScript `MINIMAX_MODELS` values for both `minimax` and `minimax-cn`
and propagate the seed into persisted/runtime provider config. Until that lands,
an explicit user config activates `reasoning_split`, while the bundled MiniMax
default does not. The focused adapter regression asserts both the listed-model
positive and unlisted-model negative cases.
